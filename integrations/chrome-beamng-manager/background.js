...
async function schedulePollingAlarm() {
  ...
  // Chrome alarms are not reliable below ~30s on MV3.
  const effectiveSeconds = Math.max(MIN_ALARM_SECONDS, pollSeconds);
  extApi.alarms.create(POLL_ALARM_NAME, { periodInMinutes: effectiveSeconds / 60.0 });
}
